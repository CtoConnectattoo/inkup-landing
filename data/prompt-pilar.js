...
// ✅ Ejemplo: gestion-consultas-clientes (Pilar)
const pilarExample = `
{
...
const pilarExampleSafe = JSON.stringify(pilarExample);
...
OBJETIVO:
Redacta un artículo PILAR que sea la guía maestra del tema (cluster), con definición + modelo mental + proceso + errores + toolkit + enlazado interno.

REFERENCIAS IMPORTANTES:
1) Plantillas de brief (Pilar, Satélite, Plantilla) — usa la sección PILAR como guía:
${briefExampleSafe}

2) Estructura brief (resumen): sigue la estructura PILAR + TL;DR + definición + diagnóstico + modelo mental + proceso + errores + cuándo sí/no + toolkit + FAQ + enlazado interno.
3) Ejemplo de post pilar (gestion-consultas-clientes):
${pilarExampleSafe}
...
- slug: extrae solo el path final de "${safe(meta.slug)}" (ej: centralizar-consultas-instagram-whatsapp)
- title: "${safe(meta.h1)}"
...
- readingTime: estima según longitud (ej: "10 min")
...
- featured: false
- content: HTML del artículo (estructura PILAR completa)
- faq: array de 12-18 objetos { question, answer }
